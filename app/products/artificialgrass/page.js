...
const currentRoute = '/products/artificialgrass'
...
export default function ArtificialGrassPage() {
  ...
  const [activeNav, setActiveNav] = useState('Artificial Grass')
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'ArtificialGrass'), (snap) => {
  ...
  const handleAddOrSave = async () => {
    ...
    try {
      ...
      if (editingProd && editingProd.id) {
        const ref = doc(db, 'ArtificialGrass', editingProd.id)
        await updateDoc(ref, prodData)
      } else {
        await addDoc(collection(db, 'ArtificialGrass'), prodData)
      }
      resetModal()
    } catch (error) {
  ...
  const deleteProduct = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'ArtificialGrass', productToDel))
      setShowDelete(false)
  ...
  const updatePriceGroup = (index, key, value) => {
  ...
  return (
    <div className="bg-gray-50 min-h-screen flex">
      ...
      <main className="flex-1 ml-0 lg:ml-72 p-4 sm:p-6 overflow-auto">
        ...
        <h2 className="text-xl sm:text-2xl font-semibold text-black mb-4">Products - Artificial Grass</h2>
        <button
            ...
            <div className="bg-white p-6 rounded-lg w-full max-w-3xl space-y-4 max-h-[90vh] overflow-y-auto">
              ...
              <div>
                ...
                {newProduct.images.map((img, i) => (
                  <div key={i} className="flex gap-2 mb-2 items-center w-full">
                    <input
                      type="text"
                    ...
                    <label
                      ...
                      className="border p-1 rounded flex-1 text-gray-900 cursor-pointer text-center select-none"
                      style={{ height: '38px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'white' }}
                    >
                      {img.src ? 'File Selected' : 'Choose File'}
                    </label>
                    <button
                      onClick={() => {
              ...
              <div>
                ...
                <button
                  onClick={() =>
                    setNewProduct({ ...newProduct, features: [...(newProduct.features || []), { icon: '', title: '', description: '' }] })
                  }
                  className="text-blue-600 underline text-sm"
                >
                  Add Feature
              ...
              <div>
                ...
                {newProduct.priceGroup.map((p, i) => (
                  <div key={i} className="flex gap-2 mb-2 items-center">
                    <input
                      ...
                      value={p.measurement || ''}
                      onChange={(e) => updatePriceGroup(i, 'measurement', e.target.value)}
                      className="border p-1 rounded flex-1 text-gray-900"
                    />
                    <input
                      type="number"
                      placeholder="Price"
                      value={p.price || ''}
                      onChange={(e) => updatePriceGroup(i, 'price', e.target.value)}
                    ...
                    <input
                      ...
                      value={p.sizeType || ''}
                      onChange={(e) => updatePriceGroup(i, 'sizeType', e.target.value)}
                ...
                <button
                  onClick={() =>
                    setNewProduct({
                      ...newProduct,
                      priceGroup: [...(newProduct.priceGroup || []), { icon: '', measurement: '', price: '', sizeType: '' }],
                    })
                  }
                  className="text-blue-600 underline text-sm"
                >
