...
const currentRoute = '/products/decorativeplants'
...
export default function ProductsPage() {
  ...
  const [activeNav, setActiveNav] = useState('Decorative Plants')
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'DecorativePlants'), (snap) => {
  ...
  const handleAddOrSave = async () => {
    ...
    try {
      ...
      if (editingProd) {
        ...
        const ref = doc(db, 'DecorativePlants', docId)
        await updateDoc(ref, prodData)
      } else {
        await addDoc(collection(db, 'DecorativePlants'), prodData)
      }
      resetModal()
    } catch (error) {
  ...
  const deleteProduct = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'DecorativePlants', productToDel))
      setShowDelete(false)
          ...
          <div className="px-4 lg:px-8 py-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex-1 lg:flex-none lg:text-center">
                <h1 className="text-2xl font-bold text-gray-900">Decorative Plants</h1>
