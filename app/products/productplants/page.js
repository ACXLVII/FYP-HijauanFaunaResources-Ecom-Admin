...
const currentRoute = '/products/productplants'
...
export default function ProductsPage() {
  ...
  const [activeNav, setActiveNav] = useState('Product Plants')
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'ProducePlants'), (snap) => {
  ...
  const handleAddOrSave = async () => {
    ...
    try {
      ...
      if (editingProd) {
        ...
        const ref = doc(db, 'ProducePlants', docId)
        await updateDoc(ref, prodData)
      } else {
        await addDoc(collection(db, 'ProducePlants'), prodData)
      }
      resetModal()
    } catch (error) {
  ...
  const deleteProduct = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'ProducePlants', productToDel))
      setShowDelete(false)
      ...
      <div className="flex flex-col lg:pl-64 w-full">
          ...
          <div className="px-4 lg:px-8 py-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex-1 lg:flex-none lg:text-center">
                <h1 className="text-2xl font-bold text-gray-900">Product Plants</h1>
        ...
        <main className="p-4 lg:p-8 flex-1 overflow-auto bg-gray-50">
          <div className="lg:hidden mb-4">
            <button
              onClick={() => setIsModalOpen(true)}
              className="w-full px-4 py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition font-semibold text-sm"
            >
              Add Product
            </button>
          </div>
