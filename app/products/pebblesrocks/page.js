...
export default function ProductsPage() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'Pebbles'), (snap) => {
  ...
  const handleAddOrSave = async () => {
    ...
    try {
      ...
      if (editingProd) {
        ...
        const ref = doc(db, 'Pebbles', docId)
        await updateDoc(ref, prodData)
      } else {
        await addDoc(collection(db, 'Pebbles'), prodData)
      }
      resetModal()
    } catch (error) {
  ...
  const deleteProduct = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'Pebbles', productToDel))
      setShowDelete(false)
