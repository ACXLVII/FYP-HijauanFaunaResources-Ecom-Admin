...
const currentRoute = '/products/pebblesrocks'
...
export default function ProductsPage() {
  ...
  const [activeNav, setActiveNav] = useState('Pebbles Rocks')
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'RocksPebbles'), (snap) => {
  ...
  const handleAddOrSave = async () => {
    ...
    try {
      ...
      if (editingProd) {
        ...
        const ref = doc(db, 'RocksPebbles', docId)
        await updateDoc(ref, prodData)
      } else {
        await addDoc(collection(db, 'RocksPebbles'), prodData)
      }
      resetModal()
    } catch (error) {
  ...
  const deleteProduct = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'RocksPebbles', productToDel))
      setShowDelete(false)
          ...
          <div className="px-4 lg:px-8 py-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex-1 lg:flex-none lg:text-center">
                <h1 className="text-2xl font-bold text-gray-900">Pebbles Rocks</h1>
